...
static void usage() {
	std::cerr
		...
		<< "  --bed FILE             Keep blocks whose pos is within BED intervals (chr start end; no header)\n"
		<< "  --target-chr STR       Scan one target block/pos vs all others (target chromosome)\n"
		<< "  --target-pos INT       Scan one target block/pos vs all others (target position; matches single pos column)\n"
