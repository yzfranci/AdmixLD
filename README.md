...
```bash
tests/test_smoke.sh
./build/admixld --vcf tests/data/ancestry_blocks.vcf --out test --min-abs-r 0.5 --max-windows 200 --intra
```

# Run in parall
```bash
# interchrom permutation test
./build/admixld --vcf tests/data/ancestry_blocks.vcf --out scan --max-windows 8000 --permute 10 --threads 4 --permute-sample 200000 --seed 123
```
